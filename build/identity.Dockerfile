...
COPY src/VelaShell.Identity/*.csproj ./src/VelaShell.Identity/
RUN dotnet restore src/VelaShell.Identity/VelaShell.Identity.csproj

COPY src/VelaShell.Identity/ ./src/VelaShell.Identity/
RUN dotnet publish src/VelaShell.Identity/VelaShell.Identity.csproj \
    -c Release -o /app --no-restore --nologo

FROM base AS final
WORKDIR /app
COPY --from=build --chown=identity:identity /app .
ENTRYPOINT ["dotnet", "VelaShell.Identity.dll"]
