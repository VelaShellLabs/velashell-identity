...
//
// ⚠️ 这个名字里的 "market" 是历史包袱,**不要"顺手改成 velashell-identity"**:
// 它是 DataProtection 的密钥环隔离键,一改就等于换了一套密钥 —— 所有人当场被登出,
// 正停在登录页的人还会拿到一个看不懂的防伪校验失败。本服务 2026-08-30 从
// velashell-markets 拆出来时,连同库名与 Issuer 一起被刻意保留成原值。
...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
       .AddCookie(options =>
       {
           ...
           options.Events.OnValidatePrincipal = async context =>
           {
               ...
               string? stamp = context.Principal?.FindFirst(IdentityClaims.SecurityStamp)?.Value;
               AccountStore accounts = context.HttpContext.RequestServices.GetRequiredService<AccountStore>();
               IdentityAccount? account = subject is null ? null : await accounts.FindByIdAsync(subject, context.HttpContext.RequestAborted);
