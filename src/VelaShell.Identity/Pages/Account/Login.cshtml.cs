...
public sealed class LoginModel(AccountStore accounts, IOptions<AccountOptions> options) : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync(CancellationToken cancel)
    {
        ...
        IdentityAccount? account = await accounts.FindByLoginAsync(Input.Login, cancel);
...
public static class SessionPrincipal
{
    ...
    public static ClaimsPrincipal Create(IdentityAccount account)
    {
        ...
        identity.AddClaim(new(IdentityClaims.SecurityStamp, account.SecurityStamp));
        return new(identity);
    }
}
